...
declare variable $codes:codes := concat($config:base,"codes/");  (: the full set of versioned codelist :)
declare variable $codes:current := concat($config:base,"currentCodes");  (: the current code set :)
declare variable $codes:metadata := doc(concat($codes:codes,"metadata.xml"))/metadata;
...
declare function codes:codelist($code as xs:string, $version as xs:string? , $lang as xs:string?)  as element(codelist)? {
  ...
  let $lists := 
    ...
    else $lists[@version = max($lists[not(@draft)]/@version/string()) ]
  return
    if (exists($lang))
    then ($lists[@xml:lang = $lang], $lists[@xml:lang = "en"]) [1]
   ...
   return 
     <tr>
        <td>{if (exists(codes:codelist($name)))
             then <a href="{$context/_root}codelist/{$name}">{$code/label/string()}</a>
...
declare function codes:code-list-metadata-as-html($context) as element(div) {
let $metadata := codes:code-metadata($context/codelist)
let $current := codes:codelist($context/codelist)
return
  <div>
    <table border="1">
       ...
       <tr><th>IATI Version</th><td>{string(($metadata/IATIVersion,"1")[1])}</td></tr>
       ...
       <tr><th>Versions</th><td>
          <table>
           <tr><th>Version</th><th>Date</th><th>Lang</th><th>Status</th><th>Description</th></tr>
           {for $codelist in codes:codelist-all($context/codelist)
            let $meta := $metadata/version[@id=$codelist/@version]
            ...
            <tr>
                ...
                <td>{if ($codelist/@version = $current/@version)
                     then "Current"
                     else if ($codelist/@draft)
                     then "Draft"
                     else ""
                    }
                </td>
                <td>{$meta/description/node()}</td>
            </tr>
           }
          </table>
         </td>
...
(: extract a consolidated list of codes from the iatistandard site 
...
let $store := xmldb:store($codes:current,"OrganisationIdentifierFull.xml",$codelist)
return
...
(: mine a set of activities for the organisations and their names 
   if more than 1 names, no name is used - maybe a set of names with a common prefix - could try to find that common stem 
...
<codelist name="OrganisationIdentifierFull" date-last-modified="current-dateTime()" version="1.0" xml:lang="en">
{
for $code in 
    distinct-values($activities/participating-org/@iati-ad:org)
    
let $codeinlist := codes:code-value("OrganisationIdentifier",$code)
return 
   if (exists($codeinlist))
   then 
     ...
     let $names :=  distinct-values($selected-orgs[text() ne ""])        
     let $prefname := 
             if (empty($names))
             then ()
             else if (count($names) = 1)
             then $names
             else  (for $n in $names order by string-length($n) return $n) [last()]
     return 
       if (exists($prefname))
       then
       <OrganisationIdentifier>
