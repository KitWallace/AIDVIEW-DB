...
import module namespace config ="http://tools.aidinfolabs.org/api/config" at "../lib/log.xqm";
import module namespace log ="http://tools.aidinfolabs.org/api/log" at "../lib/config.xqm";
     ...
     typeswitch($node)
         ...
         case element(value) return
            ...
            let $USD-value :=  
             ...
             then $value
             else activity-transform:convert-value-to-USD($value-currency, $year ,$value)
            return
            element value {
               $node/@* ,
...
:)
declare function activity-transform:convert-value-to-USD($from-code as xs:string, $year as xs:string, $amount as xs:string) {
    ...
    element iati-activity {
       ...
       return 
             element reporting-org {
                 $org/@*,
                 if ($org/@ref ne "") then attribute iati-ad:reporter {$org/@ref} else (),
                 $org/node()
             },
...
declare function activity-transform:check-aidview($activity as element(iati-activity)) as element(iati-activity) {
   element iati-activity {
      ...
      for $status in $activity/activity-status   (: should only be one :)
      return element activity-status {$status/@*,attribute iati-ad:status {$status/@code}, $status/*},
      $activity/(* except activity-status)    
    }
};


declare function activity-transform:log-message($context,$activity,$msg){
  let $update := log:write($context/corpus,"error","pipeline","activity",$activity/iati-identifier,$msg)
  return $activity
};

 declare function activity-transform:transform($activity,$context) {
 let $tactivity := $activity
 let $tactivity := util:catch( "*", activity-transform:add-default-sector-vocab($tactivity), activity-transform:log-message($context,$tactivity,"add-default-sector-vocab"))
 let $tactivity := util:catch( "*", activity-transform:convert-to-USD($tactivity), activity-transform:log-message($context,$tactivity,"convert-to-USD"))
 let $tactivity := util:catch( "*", activity-transform:add-project-value($tactivity), activity-transform:log-message($context,$tactivity,"add-project-value"))
 let $tactivity := util:catch( "*", activity-transform:apportion-sector-value($tactivity), activity-transform:log-message($context,$tactivity,"apportion-sector-value"))
 let $tactivity := util:catch( "*", activity-transform:add-DAC-3-sectors($tactivity), activity-transform:log-message($context,$tactivity,"add-DAC-3-sectors"))
 let $tactivity := util:catch( "*", activity-transform:apportion-country-value($tactivity), activity-transform:log-message($context,$tactivity,"apportion-country-value"))
 let $tactivity := util:catch( "*", activity-transform:apportion-region-value($tactivity), activity-transform:log-message($context,$tactivity,"apportion-region-value"))
 let $tactivity := util:catch( "*", activity-transform:add-org-key($tactivity), activity-transform:log-message($context,$tactivity,"add-org-key"))
 (: let $tactivity := util:catch( "*", activity-transform:add-wo-locations($tactivity), activity-transform:log-message($context,$tactivity,"add-wo-locations")) :)
 let $tactivity := util:catch( "*", activity-transform:check-aidview($tactivity), activity-transform:log-message($context,$tactivity,"check-aidview"))
