...
declare function pipeline:pipelines-as-html($context) as element(div) {
  <div>
   <table border="1">
     {for $pipeline in $pipeline:pipelines
      return
       <tr><th><a href="{$context/_root}pipeline/{$pipeline/@name}">{$pipeline/@name/string()}</a></th>
...
declare function pipeline:pipeline-as-html($context) as element(div) {
let $pipeline := pipeline:pipeline($context/pipeline)
return
  <div>
