        ...
        if ($text="") then ()
         ...
         then jxml:parse-csv-rough($text)
            ...
            return 
              ...
              then if (starts-with($token/name,"@"))
                   then attribute {substring($token/name,2)} {$token/value/string()}
                   else element {$token/name} {$token/value/string()}
...
let $request :=
   element hc:request {attribute method {"GET"}, attribute href {$url}, attribute timeout {"10"} }
